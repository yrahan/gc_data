...
gdp <- read.csv("./data/gdp.csv",header=FALSE, skip=5,nrows=215,stringsAsFactors=FALSE)

names(gdp)
head(gdp)
tail(gdp, n=10)
mygdp <- as.numeric(gsub(",", "", gdp$V5))

mean(mygdp, na.rm=TRUE)
...
gdp <- rename(gdp, c("V1"="CountryCode", "V2"="Ranking", "V4"="Name", "V5"="GDP"))
head(gdp, n=3)

gdp_education <- merge(gdp, education, by.x="CountryCode", by.y="CountryCode", all=TRUE)
names(gdp_education)
grep("[Jj]une",gdp_education$Special.Notes, value=TRUE)
grep("Fiscal year end: June",gdp_education$Special.Notes, value=TRUE)



# Question 5
# You can use the quantmod (http://www.quantmod.com/) package 
# to get historical stock prices for publicly traded companies on the NASDAQ and NYSE. Use the following code to download data on Amazon's stock price and get the times the data was sampled.
# library(quantmod)
